...
class FilipayCardService{
  ...
  isRiderCard(card: IRiderWallet | string | boolean): card is IRiderWallet {
    ...
    async UpdateIncreaseMasterCardDecreaseFilipayCard(masterCardId : string , filipayCardId : string, amount : number){

      try{
        
        const findCardIdInMasterCard : any = await MasterCardRepository.FindCardIdInMasterCard(masterCardId);
      ...
      const decreaseAmountInMasterCard = await MasterCardRepository.UpdateMasterCardBalanceByCardId(masterCardId,amount,0)
